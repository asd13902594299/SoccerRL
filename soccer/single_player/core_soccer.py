...
class World:  # multi-agent world
    ...
    @property
    def entities(self):
        return self.agents + self.landmarks + self.goals + [self.ball]
    ...
    def check_termination_conditions(self):
        # Check if the ball has scored a goal
        for goal in self.goals:
            if np.linalg.norm(self.ball.state.p_pos - goal.state.p_pos) < goal.size:
                self.termination = True
                return
        # Check if the ball is out of bounds
        if abs(self.ball.state.p_pos[0]) > self.width  or abs(self.ball.state.p_pos[1]) > self.height :
            self.termination = True
            return
        self.termination = False
    ...
    def integrate_state(self, p_force):
        for i, entity in enumerate(self.entities):
            if not entity.movable:
                continue
            entity.state.p_pos = entity.state.p_pos.astype(
                np.float64) + entity.state.p_vel * self.dt
