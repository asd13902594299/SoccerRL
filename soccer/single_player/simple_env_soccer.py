...
class SimpleEnv(AECEnv):
    ...
    def draw(self):
        ...
        # Set the field dimensions in pixels (centered)
        field_width, field_height = 1000, 600  # scaling factor: 10 pixels per unit
        field_left = (self.width - field_width) // 2
        field_top = (self.height - field_height) // 2

        # Draw soccer field boundaries
        pygame.draw.rect(self.screen, (0, 255, 0),
                         (field_left, field_top, field_width, field_height), 2)

        # Draw goals at each end of the field, centered vertically
        # in pixels (width is now horizontal thickness)
        goal_width, goal_height = 10, 60
        goal_y = field_top + (field_height - goal_height) // 2

        # Left goal
        pygame.draw.rect(self.screen, (255, 0, 0),
                         (field_left - goal_width, goal_y, goal_width, goal_height))
