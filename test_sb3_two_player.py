...
def train():
    ...
    model = PPO("MlpPolicy", env, verbose=1, device=device, learning_rate=0.0001, ent_coef=0.01, gamma=0.97, batch_size=256)
...
def eval():
    ...
    for idx, agent in enumerate(env.agent_iter()):
        ...
        pair_rewards.append(f"{agent.rjust(7, ' ')}, Reward: {reward:3.1f}")
        if len(pair_rewards) == 2:
            # Print rewards for two agents on the same line
            print(" , ".join(pair_rewards))
            pair_rewards = []
